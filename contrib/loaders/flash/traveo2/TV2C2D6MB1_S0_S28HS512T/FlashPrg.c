...
#define TGT_TRX_TYPE    CY_SPI_TRANSACTION_8S8S8S
...
static void init_variables() {
  ...
  static const cy_stc_device_config_t smif_dev_cfg_c = {
      .xipReadCmdId       = CY_SMIF_SEMP_RDID_8S8S8S_4ADR,
      .xipWriteCmdId      = CY_SMIF_SEMP_WTID_8S8S8S_4ADR,
      .writeEn            = false,
      .xipReadMode        = 0u,
      .xipWriteMode       = 0u,
      .mergeEnable        = true,
...
__attribute__((used)) int ProgramPage(unsigned long adr, unsigned long sz, const unsigned char* buf) {
  ...
  Cy_SMIF_SEMP_WriteEnable(TGT_SMIF, TGT_SLAVE, TGT_TRX_TYPE, &smif_ctxt);
  Cy_SMIF_MMIO_Program_ExMem(TGT_SMIF, TGT_SLAVE, CY_SMIF_SEMP_WTID_8S8S8S_4ADR, adr, sz, buf, CY_SMIF_BLOCKING,
