...
/** ***********************************************************************************************
 * @brief Check if target belongs specifcally to ARM_ARCH_V8M
 * @param target current target
 * @return true - target is ARM_ARCH_V8M architecture
 *************************************************************************************************/
static bool arch_armv8m(struct target *target)
{
	bool res = false;
	struct armv7m_common *armv7m_target = target_to_armv7m(target);

	if (is_armv7m(armv7m_target) && (armv7m_target->arm.arch == ARM_ARCH_V8M)) {
		LOG_DEBUG("cmsis_flash: Target has armv8m arch");
		res = true;
	}
...
/** ***********************************************************************************************
 * @brief Check if target has SecurityExtension feature.
 * 		Note: function duplicates cortex_m_has_tz() in cortex_m.c
 * @param target - current target
 * @return true - target has TZ implemented, false - TZ not implemented
 *************************************************************************************************/
static bool cortex_m_has_tz(struct target *target)
{
	...
	if (!arch_armv8m(target))
		return false;

	retval = target_read_u32(target, DAUTHSTATUS, &dauthstatus);
	if (retval != ERROR_OK) {
		LOG_WARNING("cmsis_flash: Error reading DAUTHSTATUS register");
		return false;
	}
...
static int cmsis_algo_execute(struct cmsis_flash *algo, struct target *target,
        uint32_t fn_offset, size_t timeout, int32_t *result, size_t argc, ...)
{
	bool armv8m = false;
	char* msplim;

	if (argc > 4) {
		LOG_ERROR("cmsis algorithm: wrong number of arguments given");
		return ERROR_FAIL;
	}

	/* Only ARMv6M/v7M/v8M are currently supported */
	if (!is_armv7m(target_to_armv7m(target))) {
		LOG_ERROR("cmsis algorithm: only armv6m/v7m/v8m targets are supported");
		return ERROR_TARGET_INVALID;
	} else if (arch_armv8m(target)) {
		armv8m = true;
		msplim = cortex_m_has_tz(target)? "msplim_s": "msplim";
	}

	static char *param_regs[4] = { "r0", "r1", "r2", "r3" };
	struct reg_param regs[8];
	...
	if (NULL == wa) {
		LOG_ERROR("cmsis algorithm: Working Area is not defined");
		return ERROR_FAIL;
	}
	...
	if (armv8m) {
		init_reg_param(&regs[param_cnt + 3], msplim, 32, PARAM_OUT);
	}
	...
	if (armv8m) {
		buf_set_u32(regs[param_cnt + 3].value, 0, 32, 0x00);
	}

	param_cnt += armv8m? 4:3;
...
static int cmsis_flash_erase(struct flash_bank *bank, unsigned int first, unsigned int last)
{
	...
	/* Use sector-by-sector erase if the number of sectors is less than 16.
	 * Helps to avoid timeouts if bank size is artificially limited by the config file */
	if (bank->num_sectors == last - first + 1 && algo->of_erase_chip != UINT32_MAX
		&& !algo->prefer_sector_erase && bank->num_sectors > 16) {
        ...
        uint32_t count)
{
	...
	struct reg_param reg_params[reg_count];
	...
	if (armv8m) {
		init_reg_param(&reg_params[3], msplim, 32, PARAM_OUT);
	}
...
/** ***********************************************************************************************
 * @brief Performs Flash Read operation using SEGGER's custom
 * @param bank current flash bank
 * @param buffer pointer to the buffer for data
 * @param offset starting offset in flash bank
...
static int cmsis_flash_read(struct flash_bank *bank, uint8_t *buffer, uint32_t offset, uint32_t count) {
	...
	if (algo->of_read == UINT32_MAX) {
		hr = default_flash_read(bank, buffer, offset, count);
		goto exit;
	}

	uint32_t buffer_size = target_get_working_area_avail(target);
	buffer_size = (buffer_size / algo->flash_dev.sz_page) * algo->flash_dev.sz_page;

	struct working_area *wa;
	hr = target_alloc_working_area(target, buffer_size, &wa);
	if (hr != ERROR_OK)
		goto exit;

	while (count) {
		const int32_t this_run = MIN(count, buffer_size);
		const uint32_t tgt_addr = bank->base + offset;

		int32_t result = 0;
		hr = cmsis_algo_execute(algo, target, algo->of_read, algo->flash_dev.timeout_erase, &result,
								3, tgt_addr, this_run, wa->address);
		if (hr != ERROR_OK || result != this_run)
			break;

		hr = target_read_buffer(target, wa->address, this_run, buffer);
		if (hr != ERROR_OK)
			break;
...
FLASH_BANK_COMMAND_HANDLER(cmsis_flash_bank_command)
{
	...
	/* Not memory mapped by default, can be changed using 'flash set_memory_mapped' command */
	...
	algo->of_read = cmsis_symbols[7].offset;
	algo->of_flash_device = cmsis_symbols[8].offset;
	algo->of_prg_data = cmsis_symbols[9].offset;
	...
	if (!flash_dev_found) {
		LOG_ERROR("cmsis_flash: 'FlashDevice' structure can not be found");
		hr = ERROR_IMAGE_FORMAT_ERROR;
		goto close_free_algo;
	} else {
