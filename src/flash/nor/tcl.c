 ...
 * @return ERROR_OK in case of success, ERROR_XXX code otherwise
 */
int byte_buffer_from_string(const char *str, uint8_t **buffer, size_t *size)
{
	int hr;
...
COMMAND_HANDLER(handle_flash_rmw_command)
{
	...
	COMMAND_PARSE_NUMBER(target_addr, CMD_ARGV[0], rmw_address);
...
COMMAND_HANDLER(handle_flash_set_mem_mapped_command)
{
	...
	struct flash_bank *p;
	int retval = CALL_COMMAND_HANDLER(flash_command_get_bank_probe_optional,
									  0, &p, false);
	if (ERROR_OK != retval)
		return retval;
