...
static bool is_same_data(struct target *target, struct image *image) {
	...
	for (unsigned int sect_idx = 0; sect_idx < image->num_sections; sect_idx++) {
		...
		LOG_USER_N("Verifying region (" TARGET_ADDR_FMT ", %9d)... ", section->base_address, section->size);
...
cleanup:
	LOG_USER("Mismatch");
	LOG_INFO("Data mismatch, proceeding with flash programming");
	free(data);
	return false;
}
...
skip_erase:
	...
	if (strncmp(packet, "vFlashDone", 10) == 0) {
		if(smart_program && is_same_data(target, gdb_connection->vflash_image))
			goto skip_program;
