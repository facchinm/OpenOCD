...
void command_print_sameline_noecho(struct command_invocation *cmd, const char *format, ...)
{
	char *string;
...
void command_print_noecho(struct command_invocation *cmd, const char *format, ...)
{
	char *string;
...
COMMAND_HANDLER(handle_find)
{
	...
	command_print_noecho(CMD, "%s", full_path);
	free(full_path);
...
COMMAND_HANDLER(handle_local_echo_command)
{
	 ...
	 * LOG_WARNING("DEPRECATED! The 'local_echo' command will be removed in further releases");
	 * g_local_echo = false;
	 */
