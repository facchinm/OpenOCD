...
/* Hard architectural limit in FreeRTOS kernel. */
#define FREERTOS_MAX_PRIORITIES	63
/* Guards against uninitialized SRAM causing a huge malloc() in OpenOCD. */
#define FREERTOS_MAX_TASKS		1024
...
static const struct stack_register_offset freertos_cm33_stack_offsets[] = {
...
static const struct stack_register_offset freertos_cm33_fpu_stack_offsets[] = {
...
static target_addr_t freertos_armv8_stack_align(struct target *target, const uint8_t *stack_data,
...
static target_addr_t freertos_armv8_fpu_stack_align(struct target *target, const uint8_t *stack_data,
		const struct rtos_register_stacking *stacking, target_addr_t stack_ptr)
{
...
static const struct rtos_register_stacking freertos_cm33_stacking = {
	.stack_registers_size = 0x48,									/* stack_registers_size 4 more for LR*/
	.stack_growth_direction = -1,									/* stack_growth_direction */
	.num_output_registers = ARMV7M_NUM_CORE_REGS,					/* num_output_registers */
	.calculate_process_stack = freertos_armv8_stack_align,			/* stack_alignment */
	.register_offsets = freertos_cm33_stack_offsets					/* register_offsets */
};

static const struct rtos_register_stacking freertos_cm33_fpu_stacking = {
	.stack_registers_size = 0xD0,									/* stack_registers_size 4 more for LR*/
	.stack_growth_direction = -1,									/* stack_growth_direction */
	.num_output_registers = ARMV7M_NUM_CORE_REGS,					/* num_output_registers */
	.calculate_process_stack = freertos_armv8_fpu_stack_align,		/* stack_alignment */
	.register_offsets = freertos_cm33_fpu_stack_offsets				/* register_offsets */
};
...
static int freertos_read_stack_regs_armv8m(struct rtos *rtos, int64_t stack_ptr,
	...
	uint32_t exec_lr;
	target_read_u32(target, stack_ptr + 0x04, &exec_lr);
	if(exec_lr & (1 << 4)) {
		/* Standard stack frame */
		return rtos_generic_stack_read(target, param->stacking_info_cm33, stack_ptr, reg_list, num_regs);
	} else {
		/* Extended stack frame */
		return rtos_generic_stack_read(target, param->stacking_info_cm33_fpu, stack_ptr, reg_list, num_regs);
	}
}
...
static const struct freertos_params freertos_params_list[] = {
	...
	&freertos_cm33_stacking,
	&freertos_cm33_fpu_stacking,
	},
	{
...
struct symbols {
	const char *name;
	bool optional;
	bool no_wipe; /* Skip wiping this symbol. */
};

static const struct symbols freertos_symbol_list[] = {
	{ "pxCurrentTCB", false, false },
	{ "pxReadyTasksLists", false, false },
	{ "xDelayedTaskList1", false, false },
	{ "xDelayedTaskList2", false, false },
	{ "pxDelayedTaskList", false, false },
	{ "pxOverflowDelayedTaskList", false, false },
	{ "xPendingReadyList", false, false },
	{ "xTasksWaitingTermination", true, false }, /* Only if INCLUDE_vTaskDelete */
	{ "xSuspendedTaskList", true, false }, /* Only if INCLUDE_vTaskSuspend */
	{ "uxCurrentNumberOfTasks", false, false },
	{ "uxTopUsedPriority", true, true }, /* Unavailable since v7.5.3; constant in flash - can't be wiped */
	{ "xSchedulerRunning", false, false },
	{ NULL, false, false }
};
...
static int freertos_update_threads(struct rtos *rtos)
{
	...
	/* Guard against uninitialized SRAM (leftover from a previous run, boot code, etc. */
	if (thread_list_size > FREERTOS_MAX_TASKS) {
		LOG_ERROR("FreeRTOS thread count is unreasonably large, not proceeding: %" PRIu32,
			thread_list_size);
		return ERROR_FAIL;
	}
	...
	LOG_DEBUG("FreeRTOS: Read uxTopUsedPriority at 0x%" PRIx64 ", value %" PRIu32,
										rtos->symbols[FREERTOS_VAL_UX_TOP_USED_PRIORITY].address,
										top_used_priority);
	/* Guard against uninitialized SRAM. Garbage value would cause reads beyond
	 * the array into arbitrary target memory. */
...
static int freertos_get_symbol_list_to_lookup(struct symbol_table_elem *symbol_list[])
{
	...
	*symbol_list = calloc(
	...
	for (i = 0; i < ARRAY_SIZE(freertos_symbol_list); i++) {
		...
		(*symbol_list)[i].no_wipe = freertos_symbol_list[i].no_wipe;
	}
