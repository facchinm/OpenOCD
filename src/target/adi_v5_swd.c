...
/* Holds the total number of SWD DAPs, used to track the last DAP in dap_quit()
 * and send the SWD->DORMANT or SWD->JTAG sequence */
long swd_num_daps;
...
static void swd_quit(struct adiv5_dap *dap)
{
	...
	/* Make sure sequence is sent at the last swd_quit() call,
	 * code above will fail otherwise on multidrop devices */
	if (--swd_num_daps)
		return;
