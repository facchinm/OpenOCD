...
static int add_memory_mapping(struct jim_getopt_info *goi, struct target *target);
...
int target_checksum_memory(struct target *target, target_addr_t address, uint32_t size, uint32_t *crc)
{
	...
	if(bank && !use_driver && target->type->checksum_memory)
...
COMMAND_HANDLER(handle_target_read_memory)
{
	...
	while (count > 0) {
		...
		for (size_t i = 0; i < chunk_len ; i++) {
			...
			command_print_sameline_noecho(CMD, "%s0x%" PRIx64, separator, v);
			separator = " ";
		}
...
COMMAND_HANDLER(handle_target_was_examined)
{
	...
	command_print_noecho(CMD, "%d", target_was_examined(target) ? 1 : 0);
...
COMMAND_HANDLER(handle_target_examine_deferred)
{
	...
	command_print_noecho(CMD, "%d", target->defer_examine ? 1 : 0);
...
COMMAND_HANDLER(handle_target_current_state)
{
	...
	command_print_noecho(CMD, "%s", target_state_name(target));
...
COMMAND_HANDLER(handle_target_current)
{
	...
	struct target *target = get_current_target_or_null(CMD_CTX);
	if (target)
		command_print_noecho(CMD, "%s", target_name(target));
