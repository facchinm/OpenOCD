 ...
 * using the library's v2 API.
 * Copyright (C) 2024 Michael Heimpold <mhei@heimpold.de>
 *
 * Derived from linuxgpiod.c:
 * Copyright (C) 2020 Antonio Borneo <borneo.antonio@gmail.com>
 *
 * Which in turn was largely based on sysfsgpio driver:
...
static struct gpiod_line_request *gpiod_line_request[ADAPTER_GPIO_IDX_NUM] = {};
...
{
	return gpiod_line_request_get_value(gpiod_line_request[idx],
			adapter_gpio_config[idx].gpio_num);
}

static int linuxgpiod_line_set_value(enum adapter_gpio_config_index idx, int value)
{
	return gpiod_line_request_set_value(gpiod_line_request[idx],
			adapter_gpio_config[idx].gpio_num,
			value);
}
...
static void linuxgpiod_swdio_drive(bool is_output)
{
	...
	if (is_output) {
		if (gpiod_line_request[ADAPTER_GPIO_IDX_SWDIO_DIR]) {
		...
		retval = gpiod_line_request_reconfigure_lines(gpiod_line_request[ADAPTER_GPIO_IDX_SWDIO],
...
static int linuxgpiod_reset(int trst, int srst)
{
	 ...
	 * thus handled when requesting the line.
	 */
	if (gpiod_line_request[ADAPTER_GPIO_IDX_SRST]) {
		retval1 = linuxgpiod_line_set_value(ADAPTER_GPIO_IDX_SRST, srst);
		if (retval1 < 0)
			LOG_WARNING("set srst value failed");
	}

	if (gpiod_line_request[ADAPTER_GPIO_IDX_TRST]) {
...
{
	if (gpiod_line_request[idx]) {
		gpiod_line_request_release(gpiod_line_request[idx]);
		gpiod_line_request[idx] = NULL;
	}
...
static int helper_get_line(enum adapter_gpio_config_index idx)
{
	...
	case ADAPTER_GPIO_INIT_STATE_INACTIVE:
		gpiod_line_settings_set_direction(gpiod_line_settings[idx], GPIOD_LINE_DIRECTION_OUTPUT);
		gpiod_line_settings_set_output_value(gpiod_line_settings[idx], GPIOD_LINE_VALUE_INACTIVE);
		break;
	case ADAPTER_GPIO_INIT_STATE_ACTIVE:
		gpiod_line_settings_set_direction(gpiod_line_settings[idx], GPIOD_LINE_DIRECTION_OUTPUT);
		gpiod_line_settings_set_output_value(gpiod_line_settings[idx], GPIOD_LINE_VALUE_ACTIVE);
		break;
	}
	...
	if (adapter_gpio_config[idx].init_state != ADAPTER_GPIO_INIT_STATE_INPUT) {
		...
		case ADAPTER_GPIO_DRIVE_MODE_PUSH_PULL:
			gpiod_line_settings_set_drive(gpiod_line_settings[idx], GPIOD_LINE_DRIVE_PUSH_PULL);
			break;
		case ADAPTER_GPIO_DRIVE_MODE_OPEN_DRAIN:
			gpiod_line_settings_set_drive(gpiod_line_settings[idx], GPIOD_LINE_DRIVE_OPEN_DRAIN);
			break;
		case ADAPTER_GPIO_DRIVE_MODE_OPEN_SOURCE:
			gpiod_line_settings_set_drive(gpiod_line_settings[idx], GPIOD_LINE_DRIVE_OPEN_SOURCE);
			break;
		}
	}
...
#ifdef GPIOD_LINE_BIAS_DISABLED
		gpiod_line_settings_set_bias(gpiod_line_settings[idx], GPIOD_LINE_BIAS_DISABLED);
#endif
		break;
	case ADAPTER_GPIO_PULL_UP:
#ifdef GPIOD_LINE_BIAS_PULL_UP
		gpiod_line_settings_set_bias(gpiod_line_settings[idx], GPIOD_LINE_BIAS_PULL_UP);
#else
		LOG_WARNING("linuxgpiodv2: ignoring request for pull-up on %s: not supported by gpiod v%s",
			adapter_gpio_get_name(idx), gpiod_api_version());
#endif
		break;
	case ADAPTER_GPIO_PULL_DOWN:
#ifdef GPIOD_LINE_BIAS_PULL_DOWN
		gpiod_line_settings_set_bias(gpiod_line_settings[idx], GPIOD_LINE_BIAS_PULL_DOWN);
#else
		LOG_WARNING("linuxgpiodv2: ignoring request for pull-down on %s: not supported by gpiod v%s",
			adapter_gpio_get_name(idx), gpiod_api_version());
#endif
		break;
	}
	...
	retval = gpiod_line_config_add_line_settings(gpiod_line_config[idx],
			&adapter_gpio_config[idx].gpio_num, 1, gpiod_line_settings[idx]);
	if (retval < 0) {
	...
	gpiod_line_request[idx] = gpiod_chip_request_lines(gpiod_chip[idx], req_cfg, gpiod_line_config[idx]);
	gpiod_request_config_free(req_cfg);
...
{
	LOG_INFO("Linux GPIOD JTAG/SWD bitbang driver (libgpiod v2)");
